...
export default function ProviderPage() {
  ...
  async function load() {
    // Fix: pass providerId so the server filters — no more 200-record client-side filter
    const data = await apiRequest(`/api/vehicles?providerId=${user.id}&limit=48`, { token });
      ...
      <div className="provider-pending">
        <section className="provider-pending-visual">
          ...
          <div className="provider-pending-content">
            <div className="auth-logo"><span>MR</span></div>
            <h1>{pendingStatus === "rejected" ? "Provider Access Rejected" : "Provider Access Pending"}</h1>
            <p>
              {pendingStatus === "rejected"
                ? "Your provider application was rejected. Contact admin support."
          ...
          <div className="provider-pending-card">
            ...
            <p className="auth-subtext">
              {pendingStatus === "rejected"
                ...
                : "You will be able to manage listings once approved."}
            </p>
            <ul className="pending-list">
              ...
              <li>Provider tools unlock after approval.</li>
            </ul>
            <p className="error">
              {pendingStatus === "rejected"
    ...
    <div className="container">
      <section className="card admin-header-card">
        ...
        <div className="tab-row">
          <button className={tab === "create" ? "tab-btn active-tab" : "tab-btn"} onClick={() => setTab("create")}>
      ...
      {tab === "create" ? (
        <section className="card">
          ...
          <form className="form" onSubmit={submit}>
            ...
            <select value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))}>
              {["Car", "SUV", "Bike", "E-Bike", "Scooter"].map((t) => (
                <option key={t} value={t}>{t}</option>
              ))}
            </select>

            <label>Price Per Day (USD)</label>
            <input type="number" min="1" value={form.pricePerDay} onChange={(e) => setForm((p) => ({ ...p, pricePerDay: e.target.value }))} />
        ...
        <section className="table-wrap admin-table-wrap">
          <table className="data-table admin-table">
            <thead>
              <tr>
                <th>Name</th><th>Type</th><th>City</th><th>Price</th><th>Status</th><th>Action</th>
              </tr>
            </thead>
            <tbody>
              {vehicles.length === 0 ? (
